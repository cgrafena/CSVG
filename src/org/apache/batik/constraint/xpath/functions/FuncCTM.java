...
/**
 * Get the current transformation matrix of a given SVG graphics element.
 */
public class FuncCTM extends ConstraintFunction {
    ...
    public XObject execute(XPathContext xpathContext, Vector argVec) {
            ...
            if (xo.getType() == XObject.CLASS_NODESET) {
                ...
                if (l != 0) {
                    ...
                    if (n instanceof SVGLocatable) {
                        SVGMatrix m = ((SVGLocatable) n).getCTM();
        ...
        throw new XPathException("ctm must be given a nodelist with at least one SVG graphics element.");
    }
}
