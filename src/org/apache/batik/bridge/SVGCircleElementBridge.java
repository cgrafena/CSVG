...
public class SVGCircleElementBridge extends SVGShapeElementBridge {
	...
	public void handleAnimationEvent(AnimationEvent evt) {
		...
		/*AbstractSVGAnimatedValue val;
		//val= new AbstractSVGAnimatedValue((AbstractElement)evt.getElement(), null, evt.getAttributeName());
		
		//SVGOMElement xy = (SVGOMElement)evt.getElement();
		//xy.updateAttributeValue(null,  evt.getAttributeName(), new AnimatableLengthValue(null, evt.getNewValue()));
				
		Element el = evt.getElement();
		el.setAttributeNS(null, evt.getAttributeName(), "150");
		 
             buildShape(ctx, e, (ShapeNode)node);
             handleGeometryChanged();
          */   
     	//super.handleAnimatedAttributeChanged(alv);
