...
import org.apache.batik.constraint.values.TransformListValue.SVGOMTransformList;
...
public abstract class AbstractGraphicsNodeBridge extends AnimatableSVGBridge
    ...
    public void handleAnimationEvent(AnimationEvent evt) {
        String attrName = evt.getAttributeName();
        if (attrName.equals(SVG_TRANSFORM_ATTRIBUTE)) {
            SVGTransformable e = (SVGTransformable) evt.getElement();
            SVGAnimatedTransformList tl = e.getTransform();
            String s = evt.getNewValue();
            if (s != null) {
                AffineTransform at = GraphicsNode.IDENTITY;
                if (s.length() != 0) {
                    at = SVGUtilities.convertTransform
                        (ctx, (Element) e, SVG_TRANSFORM_ATTRIBUTE, s);
                }
                SVGTransformList svgtl = new SVGOMTransformList();
                SVGTransform t = new SVGOMTransform();
                t.setMatrix(new SVGOMMatrix(at));
                svgtl.initialize(t);
                ((SVGOMAnimatedTransformList) tl).setAnimVal(svgtl);
                node.setTransform(at);
                handleGeometryChanged();
            } else {
                ((SVGOMAnimatedTransformList) tl).unsetAnimVal();
            }
        }
    }
