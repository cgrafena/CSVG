...
import org.apache.batik.anim.AnimationContext;
...
import org.apache.batik.dom.svg.SVGOMDocument.AnimAttrListener;
 ...
 */
public class BridgeContext implements ErrorConstants, CSSContext, AnimationContext {
    ...
    /**
     * Event handler for animation of XML attributes.
     */
    protected AnimatedAttributeChangedListener animatedAttributeChangedListener;
    
    public MyJSVGCanvas mycanvas;
     ...
     */
    public BridgeContext(UserAgent userAgent,                         
                         DocumentLoader documentLoader, MyJSVGCanvas canvas) {
        this.userAgent = userAgent;
        this.viewportMap.put(userAgent, new UserAgentViewport(userAgent));
        this.interpreterPool = interpreterPool;
        this.documentLoader = documentLoader;
    ...
    protected void initializeDocument(Document document) {
         ...
         // xgx: add constraint code
    	 
    	 if (mycanvas != null) {
    		 final ConstraintsAnimationEngine ae = new ConstraintsAnimationEngine(doc, this);
        	 doc.setAnimationEngine(ae);
                  
                 mycanvas.addGVTTreeRendererListener(new GVTTreeRendererListener() {
             ...
             animatedAttributeChangedListener = new AnimatedAttributeChangedListener();
             ae.addEventListener(animatedAttributeChangedListener);
    	 }
        ...
        if (ce == null) {
        	//System.out.println("bridge creates a new constraint engine");
            ce = new ConstraintEngine(doc);
            doc.setConstraintEngine(ce);
        }
    }
    ...
    public UpdateManager getUpdateManager() {
    	...
    	//else
    	//	return mycanvas.getUpdateManager();
    }
     ...
     * registration.
     */
    public void storeEventListener(EventTarget t,
                                      String s,
                                      EventListener l,
                                      boolean b) {
        synchronized (eventListenerSet) {
    ...
    public void dispose() {
        ...
        if (document != null) {
	        ...
	        ConstraintEngine constraintEngine = svgDocument.getConstraintEngine();
	        if (constraintEngine != null) {
	            constraintEngine.dispose();
	            svgDocument.setConstraintEngine(null);
	        }
    ...
    protected static BridgeUpdateHandler getBridgeUpdateHandler(Node node) {
    	// xgx: add constraint code 
    	if (node instanceof org.apache.batik.constraint.ConstraintElement) {
             // XXX review
             return (BridgeUpdateHandler) ((org.apache.batik.constraint.ConstraintElement) node).getBridge();
         }
    ...
    /**
     * xgx: add constraint code
     * The AnimationEventListener invoked when animated XML attribute values
     * are changed on a particular element.
     */
    protected class AnimatedAttributeChangedListener implements AnimationEventListener {
        
        /**
         * Handles an animated XML attribute being updated.
         */
        public void animatedValueUpdated(AnimationEvent evt) {
            Element elem = evt.getElement();
            SVGContext ctx = getSVGContext(elem);
            if (evt.getAttributeType() != ConstraintsAnimationEngine.ATTRIBUTE_TYPE_CSS
                    && ctx != null && (ctx instanceof BridgeUpdateHandler)) {
                ((BridgeUpdateHandler) ctx).handleAnimationEvent(evt);            	
            }
        }
    }
    ...
    public static synchronized List getGlobalBridgeExtensions() {
        ...
        // xgx: add constraint bridge extension
        org.apache.batik.constraint.BridgeExtension bec = new org.apache.batik.constraint.BridgeExtension();
        globalExtensions.add((BridgeExtension)bec);
        // xgx end
        return globalExtensions;
    }
